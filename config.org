...
Ignore macOS accessibility queries. This avoids repeated emacs-mac crashes in
the AppKit accessibility keymap path while searching Org-roam notes.
...
No automatic spell checking in Org. It takes too long on larger files, and it
errors when Hunspell dictionaries are missing.
#+BEGIN_SRC emacs-lisp
...
Use ~counsel-rg~ for live partial completion while searching notes. A plain grep
fallback remains available on ~SPC r R~ if the live UI starts acting up again.
...
Open the backlinks buffer as soon as an Org-roam file is visited. Doom already
adds a switch-buffer hook for this, but running the manager here restores the
...
Open PDFs in Evince when pressing RET in dired
...
rename the feeds (shown under the "feed" field when opening item from said feed)
#+BEGIN_SRC emacs-lisp
